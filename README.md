...
- 지수 데이터 등록 API
  - existsBy 쿼리를 통해 중복된 값이 들어오는지 사전 검증 로직을 구현하고 명확한 에러 메시지를 반환하도록 예외 처리 강화
- 지수 데이터 삭제 API
  - ID 기반의 안전한 삭제 로직 구현
  - 무조건적인 삭제(delete) 대신 findById로 먼저 데이터가 실제 존재하는지 확인하고, 부재 시 예외 처리하는 안전한 흐름 구성
  - 영속성 전이(Cascade) 및 고아 객체 관리 설정을 적용하여, 부모 객체인 지수 정보가 삭제될 때 연관된 하위 지수 데이터들이 데이터베이스 상에서 함께 제거되도록 설계하여 데이터 무결성 보장
...
- 자동 연동 스케줄러 및 배치
  ...
  - 자동 실행 결과에 따라 연동 작업 성공 및 실패 이력 저장 처리
